...
const EditSessionModal: React.FC<EditSessionModalProps> = ({ isOpen, onClose, onSave, onDelete, booking, onAddRecurring }) => {
  ...
  useEffect(() => {
    if (booking) {
      ...
      // Reset recurring fields each time a new booking is edited
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Logic for creating a NEW recurring booking from a single instance
    if (isRecurring && !booking.recurringRuleId) {
        ...
        if(new Date(seriesStartDate) >= new Date(seriesEndDate)) {
            setError("Series end date must be after the start date.");
            return;
        }
        ...
        if (success) {
            // Also delete the original single booking that this was based on
            onDelete();
            onClose();
        }
    } else {
        // Updating a single booking (or an exception from a series)
        onSave({
            groupId,
            courtId: Number(courtId),
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center">
                    <input 
                      ...
                      disabled={!!booking.recurringRuleId}
                ...
                {booking.recurringRuleId && <p id="recurring-desc" className="text-xs text-gray-500 ml-7">This session is already part of a series. To modify recurrence, edit the series.</p>}
