...
const GroupSidebar: React.FC<GroupSidebarProps> = ({ selectedDate }) => {
    ...
    const SectionToggle: React.FC<{title: string, count: number, isOpen: boolean, onToggle: () => void}> = ({ title, count, isOpen, onToggle }) => (
         <button onClick={onToggle} className="w-full flex justify-between items-center text-left p-2 rounded hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500">
    ...
    return (
        <div className="bg-white rounded-lg shadow-md flex flex-col h-64 lg:h-full lg:max-h-[calc(100vh-12rem)]">
