...
export const printSchedule = (
  ...
  const printableHTML = generateScheduleHTML(bookings, blockedSlots, groups, startDate, endDate);
...
const generateAnalyticsHTML = (analytics: any): string => {
    let heatmapHTML = `<h3 class="text-lg font-bold mt-4 mb-2">Court Utilization Heatmap</h3><div class="grid grid-cols-7 gap-1"><div></div>`
    + analytics.courts.map((c:number) => `<div class="font-bold text-center text-sm">Court ${c}</div>`).join('')
    + analytics.heatmapData.map((row: any) => 
        `<div class="font-bold text-right pr-2 text-sm">${row.hour}:00</div>` 
        + row.utilization.map((u:number) => `<div class="h-8 border" style="background-color: rgba(37, 99, 235, ${u});"></div>`).join('')
    ).join('') + `</div>`;

    let coachHTML = `<h3 class="text-lg font-bold mt-6 mb-2">Coach Hours</h3><table class="w-full border-collapse"><thead><tr class="bg-gray-100"><th class="p-2 border text-left text-sm">Coach</th><th class="p-2 border text-left text-sm">Total Hours</th></tr></thead><tbody>`
    + analytics.coachStats.map((stat:any) => `<tr><td class="p-2 border">${stat.coachName}</td><td class="p-2 border">${stat.totalHours.toFixed(2)}</td></tr>`).join('')
    + `</tbody></table>`;

    let courtHTML = `<h3 class="text-lg font-bold mt-6 mb-2">Court Usage</h3><table class="w-full border-collapse"><thead><tr class="bg-gray-100"><th class="p-2 border text-left text-sm">Court</th><th class="p-2 border text-left text-sm">Total Hours Booked</th></tr></thead><tbody>`
    + analytics.courtStats.map((stat:any) => `<tr><td class="p-2 border">Court ${stat.courtId}</td><td class="p-2 border">${stat.totalHours.toFixed(2)}</td></tr>`).join('')
    + `</tbody></table>`;
     ...
     return `
        ...
        <html>
        <head>
            <title>KTK Connect Analytics Report</title>
            <script src="https://cdn.tailwindcss.com"></script>
            <style>
                @media print {
                    body { -webkit-print-color-adjust: exact; }
        ...
        <body class="p-8 font-sans">
            <div class="flex justify-between items-center mb-8">
                <div>
                    <h1 class="text-3xl font-bold">KTK Connect Analytics</h1>
                    <p class="text-gray-600">Report for ${analytics.dateRange}</p>
                </div>
                <p class="text-sm text-gray-500">Generated on: ${new Date().toLocaleString()}</p>
            </div>
            ${heatmapHTML}
...
export const printAnalytics = (analytics: any) => {
    const printableHTML = generateAnalyticsHTML(analytics);
    const printWindow = window.open('', '_blank');
    if (printWindow) {
        printWindow.document.write(printableHTML);
        printWindow.document.close();
